app.import(function(lib, game, ui, get, ai, _status, app) {
	...
	/*-----------------分割线-----------------*/
	// 全局技能代码
			if (lib.config.extension_手杀ui_yangShi == 'off') {
	...
	controlElement: {
		...
		showSkills.forEach(function(item) {
			...
			// 主动技按钮最大显示文本数量
			// 咸鱼大佬提供代码
			node = ui.create.div('.skillitem',self.node.enable, get.translation(item. name).slice(0,2));
			// 修改上方 (0, 2) 中的 2 即可限制对应显示字数
		...
		ui.skillControl.node.enable.style.width = ui.skillControl.node.enable.childNodes.length > 2
			? '180px' : ui.skillControl.node.enable.childNodes.length > 0
			? '90px' : '0px';
