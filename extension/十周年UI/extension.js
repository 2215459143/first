		...
		// 虽然上面的 game.addVideo 提供了好几个参数，但是没啥用，因为videoContent里的update缺只给了1个参数。
				...
				Player.useCard = function () {
					...
					event._oncard = function (card, player) {
						...
						/*-----------------分割线-----------------*/
		// 修复射却问题，方法来自咸鱼大佬
		// if (this._oncard2) this._oncard2(this.card, player);
...
// 新样式装备栏，Engj.K大佬提供代码
						...
						for (i = 0; i < e.length; i++) {
							if (i == 0) {
								if (colour == 'heart' || colour == 'diamond') {
									...
									e[i].style.color = "#E938E3"; // 装备字体颜色显示
				...
				/*-----------------分割线-----------------*/
					...
					if (lib.config['extension_动皮换肤_SSEquip']) {
						var ele = card.getElementsByClassName("name2");
						if (!(ele.length > 1)) {
							var e = ele[0].children;
							if (!(e[0].nodeName == "IMG")) {
								var colour = card.getAttribute("data-suit");
								var subype = card.getAttribute("data-card-subype");
								var cardName = card.getAttribute("data-card-name");
								for (var i = 0; i < e.length; i++) {
									if (i == 0) {
										...
										if (subype == "equip3" || subype == "equip4") {
											...
											newele.setAttribute("src",eng.url + "/images/ass/" + b + "1.png")
											newele.style.height = "80%";
											newele.onerror = function () {
												this.src = eng.url + "/images/ass/weizhi.png";
												this.onerror = null;
											}
								...
								if (!(subype == "equip3" || subype == "equip4")) {
									var newele = document.createElement("img");
									newele.setAttribute("src",eng.url + "/images/ass/" + cardName + ".png")
									newele.style.height = "50%";
									newele.style.width = "10%";
									newele.onerror = function () {
										this.src = eng.url + "/images/ass/weizhi.png";
										this.onerror = null;
									}
									e[0].parentNode.insertBefore(newele,e[0]);
									if (subype == "equip4") card.style.left = "55px";
								}
							}
						}
					}
			...
			lib.element.player.$disableEquip = function(skill){
				...
				if (lib.config['extension_动皮换肤_SSEquip']) {
					...
					if (!(ele.length > 1)) {
						var e = ele[0].children;
						var subype = card.getAttribute("data-card-subype");
						var b = subype == "equip3"? "+" : subype == "equip4" ? "-" : false;
						for (var i = 0; i < e.length; i++) {
							if (!(e[0].nodeName == "IMG")) {
								if (b == "+" || b == "-") {
									console.info(e[i])
									var newele = document.createElement("img");
									newele.setAttribute("src",eng.url + "/images/ass/" + b + "1.png")
									newele.style.height = "60%";
									newele.onerror = function () {
										this.src = eng.url + "/images/ass/weizhi.png";
										this.onerror = null;
									}
									e[i].parentNode.insertBefore(newele,e[i]);
									e[i].parentNode.removeChild(e[i+1]);
									continue;
								}
							}
							...
							else if (b == "-") card.style.left = "50px";
							e[i].style.textShadow = "1px 0 0 black, 0 1px 0 black, -1px 0 0 black, 0 -1px 0 black";
							e[i].style.fontSize = b == false ? "13px" : "10px";
						}
					}
				}
...
//我自己写的			// 不同样式卡牌素材调用
				if (lib.config.extension_十周年UI_originCardPath == '1') {
					var prefix = decadeUIPath + 'image/card/'; }
				if (lib.config.extension_十周年UI_originCardPath == '2') {
					var prefix = decadeUIPath + 'image/大将军卡牌/'; }
				if (lib.config.extension_十周年UI_originCardPath == '3') {
					var prefix = decadeUIPath + 'image/果杀卡牌/'; }
				if (lib.config.extension_十周年UI_originCardPath == '4') {
					var prefix = decadeUIPath + 'image/大将军卡牌2/'; }
					cards.READ_OK = true;
					if (format == null)
						format = 'webp';
					if (format === 'off')
...
precontent:function(){
	...
	if (lib.config['extension_' + decadeUIName + '_eruda']) { // 启用控制台
	...
	window.decadeModule = (function(decadeModule){
		...
		decadeModule.init = function () {
			...
			if (lib.config.extension_十周年UI_newDecadeStyle == 'on') {
				this.css(decadeUIPath + 'player1.css'); }
			if (lib.config.extension_十周年UI_newDecadeStyle == 'off') {
				this.css(decadeUIPath + 'player2.css'); }
			if (lib.config.extension_十周年UI_newDecadeStyle == 'othersOn') {
				this.css(decadeUIPath + 'player3.css'); }
			...
			if (!(lib.config['extension_动皮换肤_SSEquip'])) {
				...
				this.css(decadeUIPath + 'equip_eng.css');
			}
...
config:{
	originCardPath: {
		...
		update:function() {
			if (window.decadeUI) ui.arena.dataset.originCardPath = lib.config['extension_十周年UI_originCardPath'];
		},
	},
	eruda:{
		name: '调试助手(开发用)',
	...
	shadowStyle: {
		name: '特效切换(新手杀有效)',
		intro: '切换局内阴影动态特效与人物弹出文字的样式，目前只有新手杀样式可用',
		init: 'on',
		item:{
			on: '原样式',
			off: '新样式',
